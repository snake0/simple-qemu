...
void kvm_run_vm(struct kvm *kvm) {
  ...
  for (i = 0; i < kvm->vcpu_number; i++) {
    if (pthread_create(&(kvm->vcpus[i]->vcpu_thread),
                       (const pthread_attr_t *)NULL,
                       kvm->vcpus[i]->vcpu_thread_func, kvm->vcpus[i]) != 0) {
      perror("can not create vcpu thread");
      exit(1);
    }
  }
  for (i = 0; i < kvm->vcpu_number; i++) {
    if (pthread_join(kvm->vcpus[i]->vcpu_thread, NULL) != 0) {
      perror("can not join vcpu thread");
      exit(1);
    }
  }
}
